...
define(['app', 'text!templates/activity.hbs', 'text!templates/activityStream.hbs', 'pages/activity', 'httpClient', 'Q'],
    function(app, activityTemplate, activityItemStreamTemplate, activity, httpClient, Q) {
        ...
        App.ActivityRoute = Ember.Route.extend({
            model: function() {
                ...
                model.set('search',"");
                model.status="";
                return this.controllerFor('activity').getActivities();
            },
            ...
            actions: {
                reload: function() {
                    var self = this;
                    var model = self.controllerFor('activity').get('model');
                    model.set('status', '');
                    model.set('start',0);
                    model.formattedActivities.clear();
                    model.activities.splice(0);
                    if(model.filter.length!=0)
                    {
                      ...
                      self.controller.postKey();
                        ...
                        self.controllerFor('activity').getActivities();
                    }
                }
            }
        ...
        App.ActivityController = Ember.ObjectController.extend({
            ...
            postKey: function() {
                ...
                if (filter != '') {
                    endPoint="/knowledgecenter/cclom/activities/search/"+filter+"?restrict=entitled&userEmailId=" +app.getEmailId();
                } else {
                    endPoint = "/knowledgecenter/cclom/aseread/all/entitledActivities/from/0/to/50/"+app.getEmailId();
                }

                   httpClient.get(endPoint).then(function(response) {

                      //Error Handling for Reponse of 200E
                    if(response.code=="200E"){
                         model.set('status', 'We are unable to fetch your activities, please try it later.');
                        return model;
                    }
            ...
            getActivities: function() {
                var endPoint = "/knowledgecenter/cclom/aseread/all/entitledActivities/from/0/to/50/"+app.getEmailId();;
                var self = this;
                ...
                return this.getAllActivities().then(function(activities) {
                    ...
                    model.set("status", " We are unable to fetch activities, please try it later");
                    return model;
                });
            },
           
            getAllActivities: function() {
                /* Get the user's Entitled Activity List */
                var self=this;
                var endPoint = "/knowledgecenter/cclom/aseread/all/entitledActivities";
                if (model.formattedActivities.length == 0) {
            ...
            //item.title=item.title+" published="+item.published; //debug statement
            ...
            filterScroll:function(key,context)
            {
                var self=this;
                var typeArr = [];
                var model=this.controllerFor('activity').get('model');
                ...
                var endPoint = "/knowledgecenter/cclom/activities/filter?userEmailId=" +app.getEmailId()+"&type=" + typeStream+"&restrict=entitled";
